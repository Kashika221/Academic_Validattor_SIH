...
def create_flask_app():
    """Create a Flask app for hosting the service"""
    ...
    def initialize_verifier():
        ...
        try:
            ...
            logger.error(f"Failed to initialize verifier: {str(e)}")
            raise
    
    @app.teardown_appcontext
    def cleanup_verifier(exception):
        if verifier:
            verifier.cleanup()
    ...
    def verify_certificate_api():
        ...
        try:
            ...
            if 'base_certificate' not in request.files:
                return jsonify({
                    "success": False,
                    "message": "No base certificate file provided"
                }), 400
            
            cert_file = request.files['certificate']
            base_file = request.files['base_certificate']
            
            if cert_file.filename == '' or base_file.filename == '':
                return jsonify({
                    "success": False,
            ...
            cert_path = os.path.join(verifier.temp_dir, f"upload_cert_{cert_filename}")
            base_path = os.path.join(verifier.temp_dir, f"upload_base_{base_filename}")
            
            cert_file.save(cert_path)
            base_file.save(base_path)
            
            # Verify certificate
            result = verifier.verify_certificate(cert_path, base_path)
